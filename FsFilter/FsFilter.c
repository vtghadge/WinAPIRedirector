...
MINIFILTER_GLOBAL_DATA g_globalData;
...
--*/
{
    ...
    UNICODE_STRING usCommportName = EMPTY_UNICODE_STRING;
    ...
    if (!NT_SUCCESS(status))
    {
        FltUnregisterFilter(g_globalData.pFltFilterHandle);
        return status;
    }

    RtlInitUnicodeString(&usCommportName, SERVER_COMM_PORT);
    InitializeObjectAttributes(&oa, &usCommportName, OBJ_KERNEL_HANDLE | OBJ_CASE_INSENSITIVE, NULL, pSecurityDesc);

    ntStatus = FltCreateCommunicationPort(
        g_globalData.pFltFilterHandle,
        &g_globalData.pServerPort,
        &oa,
        NULL,
        &MinifilterCommportConnect,
        &MinifilterCommportDisconnect,
        &MinifilterCommportMessage,
        MINIFILTER_COMMPORT_MAX_CONNECTIONS
    );
    ...
    if (!NT_SUCCESS(ntStatus))
    {
        FltCloseCommunicationPort(g_globalData.pServerPort);
        FltUnregisterFilter(g_globalData.pFltFilterHandle);
        return status;
    }
    
    status = FltStartFiltering(g_globalData.pFltFilterHandle);
    if (!NT_SUCCESS(status))
    {

        UnRegisterProcessCreateCallbacks();
        FltCloseCommunicationPort(g_globalData.pServerPort);
        FltUnregisterFilter(g_globalData.pFltFilterHandle);
        return status;
    }
